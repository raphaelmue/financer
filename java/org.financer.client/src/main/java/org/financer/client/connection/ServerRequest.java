...
public class ServerRequest<T> implements Runnable {

    private static final String DEFAULT_HOST = "https://api.financer-project.org/api/" +
            ServerRequest.class.getPackage().getImplementationVersion();
    ...
    public ServerRequest(RequestConfig requestConfig, RestCallback<T> restCallback) {
        this(requestConfig, null, restCallback);
    }

    public ServerRequest(RequestConfig requestConfig, Class<T> type, RestCallback<T> restCallback) {
    ...
    public static void setLocalStorage(LocalStorage localStorage) {
        ServerRequest.localStorage = localStorage;
    }

    public static void setApplication(Application application) {
        ServerRequest.application = application;
    }
    ...
    private HttpUrl buildUrl() {
        HttpUrl.Builder urlBuilder = HttpUrl.parse(buildUrlWithPathParameters()).newBuilder();
        requestConfig.getQueryParameters().forEach(urlBuilder::addQueryParameter);

        return urlBuilder.build();
    }

    private String buildUrlWithPathParameters() {
        String host = DEFAULT_HOST;
        if (localStorage.contains("host")) {
            host = localStorage.readObject("host");
        }

        String url = host + requestConfig.getPath();

        // replace path parameters with actual value and remove parameter from requestConfig.getQueryParameters()
        for (Map.Entry<String, String> entry : requestConfig.getQueryParameters().entrySet()) {
            url = url.replace("{" + entry.getKey() + "}", entry.getValue());
            requestConfig.getQueryParameters().remove(entry.getKey());
        }
        return url;
    }
}
